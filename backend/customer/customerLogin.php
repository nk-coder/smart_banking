  ...
  $password = md5($_POST["password"]); //encript password by using md5 hasing algorithm
    ...
    if(empty($username) || empty($password)){
      ...
      //Check account approved or not-First check NO
       $check_approved_query = mysqli_query($con, "SELECT * FROM customers WHERE username='$username' AND password='$password' AND approved='NO' ");
      $check_found_or_not = mysqli_num_rows($check_approved_query); 

      //Check account approved or not-Second check YES
      ...
      if ($verification_key_found_or_not == 1) {
        ...
        array_push($message_array, "Sorry your account still not approved. please wait...");
      }else{
...
<head>
  ...
  <title>Smart Banking-Customer Login</title>
    ...
    <div class="container">
      <form class="form-login" action="" method="POST">
        ...
        <h4 style="text-align: center;">
          ...
          <?php if(in_array("Sorry your account still not approved. please wait...",$message_array)) echo "<span style='color: #e74c3c;'>Sorry your account still not approved. please wait...</span> <br>";?>
          <?php if(in_array("Please Verifiy your account",$message_array)) echo "<span style='color: #e74c3c;'>Please Verifiy your account</span> <br>";?>
        </h4>
